...
spec:
  ...
  state:
    - name: working-directory
      ...
      onRunEnd: discard   # or commit — files survive stop
  actors:
    - name: agent
      kind: Agent
      role: worker
...
- `onRunEnd` is `discard` (drop overlay at stop unless committed) or `commit` (overlay becomes the Branch head at stop). Catalog accepts `commit`; use it when a Drive file must survive stop.
